...
syn include syntax/doxygen.vim
...
if s:strict_blocks
  ...
  " NAMESPACE: namespace my\foo: {{{2
  syn keyword phpStructure contained namespace
      \ nextgroup=phpDefineNamespaceName skipwhite skipempty

  syn match phpDefineNamespaceName /\(\\\|\h\w*\)*\h\w*/ contained

    " USE: use my\foo as MyFoo: {{{3
    syn keyword phpStructure contained use
	  \ nextgroup=@phpClDefineNamespaceUse skipwhite skipempty

    syn cluster phpClDefineNamespaceUse add=phpDefineNamespaceUseName
    syn match phpDefineNamespaceUseName /\(\\\|\h\w*\)*\h\w*/ contained contains=@phpClStructures
	  \ nextgroup=phpDefineNamespaceUseAs,@phpClDefineNamespaceUse skipwhite skipempty

    syn keyword phpDefineNamespaceUseAs contained as
	  \ nextgroup=phpDefineNamespaceUseAsName skipwhite skipempty
    syn match phpDefineNamespaceUseAsName /\h\w*/ contained
	  \ nextgroup=@phpClDefineNamespaceUse skipwhite skipempty

    syn cluster phpClDefineNamespaceUse add=phpDefineNamespaceUseComma
    syn match phpDefineNamespaceUseComma /,/ contained
          \ nextgroup=@phpClDefineNamespaceUse skipwhite skipempty

    hi link phpDefineNamespaceUseAs phpStructure
  ...
  " II MATCH: class myFoo <extends baseFoo> implements foo, Iterator { }: {{{3
      ...
      syn keyword phpStructure contained extends
            \ nextgroup=phpDefineClassExtendsName skipwhite skipempty

      syn match phpDefineClassExtendsName /\(\\\|\h\w*\)*\h\w*/ contained contains=@phpClStructures
	    \ nextgroup=@phpClDefineClassBlock skipwhite skipempty
  ...
  " III MATCH: class myFoo extends baseFoo <implements foo, Iterator> { }: {{{3
      ...
      syn match phpDefineClassImplementsName /\(\\\|\h\w*\)*\h\w*/ contained contains=@phpClStructures
            \ nextgroup=@phpClDefineClassImplements skipwhite skipempty
